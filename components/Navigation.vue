...
import { MagnifyingGlassIcon, CalendarIcon, Bars3Icon, XMarkIcon } from '@heroicons/vue/20/solid'

const route = useRoute()
const links = ['tasks', 'templates']
</script>

<template>
  <Disclosure as="nav" class="bg-gray-800 sticky top-0 z-30" v-slot="{ open }">
          ...
          <div class="flex-shrink-0">
            <NuxtLink to="/">
              <img class="block h-8 w-auto lg:hidden"
                src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=500" alt="Daily" />
          ...
          <div class="hidden lg:ml-6 lg:block">
            <div class="flex space-x-4">
              ...
              <NuxtLink v-for="link in links" :to="'/' + link"
                :class="['rounded-md px-3 py-2 text-sm font-medium', route.name === link ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white']">
    ...
    <DisclosurePanel class="lg:hidden">
      <div class="space-y-1 px-2 pb-3 pt-2">
        <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
        <NuxtLink v-for="link in links" :to="'/' + link" :class="['block rounded-md px-3 py-2 text-base font-medium', route.name === link ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white']">
          {{ link.charAt(0).toUpperCase() + link.slice(1) }}
        </NuxtLink>
      </div>
